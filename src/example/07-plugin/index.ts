import { controller, Daruk, DarukServer, get, inject, plugin, provide, type } from '../..';
...
const ParamReflect = new ReflectList<
  any,
  string,
  { index: number; name: string; type: DarukType.Constructor },
  'param'
>();

export default function Param(name: string, type: DarukType.Constructor = String) {
...
@plugin()
class ParamPlugin implements DarukType.PluginClass {
...
const main = async () => {
  try {
    ...
    class MainController {
      ...
      public async findByName(ctx: DarukType.DarukContext, _: any, @Param('name') name: string) {
